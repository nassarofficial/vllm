...
class GraniteDoclingHybridDummyInputsBuilder(
    ...
    def get_dummy_mm_data(
        ...
        h, w = self.info._get_image_size(hf_processor.image_processor)
        side = max(h, w)
...
class GraniteDoclingHybridMultiModalProcessor(
    ...
    def _call_hf_processor(
        ...
        # The HF processor handles channels-last upstream — pass through.
        ...
        if pv.ndim == 5:
            ...
            if existing_pam is not None and existing_pam.dim() == 2:
                ...
                processed_outputs.pop("pixel_attention_mask")
            else:
        ...
        # is valid. Emit an all-ones per-tile spatial mask so downstream code
        # has the conventional ``[total_patches, H, W]`` shape; the vision
        # tower itself ignores it on the uint8 path.
...
class GraniteDoclingHybridTextModel(GraniteMoeHybridModel):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # ``deepstack_attn_layers`` is set on the (parent) GraniteDoclingHybrid
        # config; when this text model is constructed standalone with the
        # GraniteMoeHybrid config alone we degrade gracefully to no-op
        # injection (the base forward path is preserved).
        ...
        self._deepstack_layer_to_slot: dict[int, int] = {
            layer_idx: slot
            for slot, layer_idx in enumerate(self.deepstack_attn_layers)
        }
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            # DeepStack residual injection at image-token positions. Restricted
            # to prefill: ``deepstack_input_embeds`` is ``None`` during decode
            # (because no visual tokens enter the new-token slice).
            if deepstack_input_embeds is not None:
                slot = self._deepstack_layer_to_slot.get(i)
                if slot is not None:
                    key = f"deepstack_input_embeds_{slot}"
                    if key in deepstack_input_embeds:
                        ds = deepstack_input_embeds[key]
                        hidden_states = hidden_states + ds.to(
...
class GraniteDoclingHybridModel(nn.Module):
    ...
    def image_pixels_to_features(
        ...
        * float — already-normalized pixel values from the HF processor.
        ...
        Zero-padded sub-images (introduced when batching variable patch
        counts) are filtered out before the vision encoder runs.
        """
        vision_dtype = self.vision_model.embeddings.patch_embedding.weight.dtype
        nb_values_per_image = pixel_values.shape[1:].numel()

        if pixel_values.dtype == torch.uint8:
            # Filter zero-padded sub-images by examining raw bytes (a genuine
            # all-black tile is never produced by the image processor — every
            # crop is rescaled to ``image_size`` and at least one pixel is
            # nonzero after the normalization step that runs on GPU below).
            real_images_inds = pixel_values.sum(dim=(-1, -2, -3)) != 0
            if not real_images_inds.all():
                real_images_inds = (
                    (pixel_values == 0).sum(dim=(-1, -2, -3)) != nb_values_per_image
                )
            pixel_values = pixel_values[real_images_inds].contiguous()

            pixel_values = pixel_values.to(dtype=vision_dtype) / 255.0
            image_mean = getattr(self.config.vision_config, "image_mean", None)
            image_std = getattr(self.config.vision_config, "image_std", None)
            ...
            # GotOcr2 crops tiles to exactly the ViT resolution — every pixel
            # is valid so we don't need a patch_attention_mask.
            return self.vision_model(
                pixel_values=pixel_values, patch_attention_mask=None
            )

        # Float path — pre-normalized by the HF processor.
        pixel_values = pixel_values.to(dtype=vision_dtype)
        real_images_inds = (
            pixel_values == 0.0
        ).sum(dim=(-1, -2, -3)) != nb_values_per_image
        pixel_values = pixel_values[real_images_inds].contiguous()
...
class GraniteDoclingHybridForConditionalGeneration(
    ...
    def get_mamba_state_shape_from_config(cls, vllm_config: VllmConfig):
        ...
        temp = copy.deepcopy(vllm_config)
        temp.model_config.hf_config = text_config
        return GraniteMoeHybridForCausalLM.get_mamba_state_shape_from_config(temp)
    ...
    def get_mamba_state_dtype_from_config(cls, vllm_config: VllmConfig):
        ...
        temp = copy.deepcopy(vllm_config)
        temp.model_config.hf_config = text_config
        return GraniteMoeHybridForCausalLM.get_mamba_state_dtype_from_config(temp)
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # Configure the hybrid mamba+attention cache before the LM is built.
        ...
        with self._mark_composite_model(
            ...
            # Deferred DeepStack scratch buffers — one per tap. We create them
            # lazily on the right device/dtype during the first prefill (see
            # ``_ensure_deepstack_buffers``). Initialising to ``None`` here
            # avoids burning ``max_num_batched_tokens * hidden_size`` of GPU
            # memory for non-DeepStack checkpoints sharing this class.
            self._deepstack_buf_capacity: int = 0
            self._deepstack_buffers: list[torch.Tensor] = []
            if self.use_deepstack:
                self._deepstack_max_tokens = (
                    vllm_config.scheduler_config.max_num_batched_tokens
                )
            else:
                self._deepstack_max_tokens = 0
    ...
    def _parse_and_validate_image_input(
        ...
        if pixel_attention_mask is None:
            # GotOcr2 doesn't emit a mask — every cropped tile is fully valid.
    ...
    def _process_image_input(
        ...
        last_hidden, intermediate = self.model.image_pixels_to_features(
            pixel_values,
            pixel_attention_mask=pixel_attention_mask,
        )
    ...
    def _ensure_deepstack_buffers(
        self, num_tokens: int, ref: torch.Tensor
    ) -> None:
        """Make sure we own ``deepstack_num_level`` buffers ≥ ``num_tokens``.

        We allocate lazily on the first prefill so the buffers inherit the
        runtime device/dtype of the LM — avoids both a fp32-CPU buffer growing
        on demand AND the cross-device add that would otherwise happen.
        """
        if not self.use_deepstack:
            return
        device = ref.device
        dtype = ref.dtype
        cap = max(num_tokens, self._deepstack_max_tokens)
        if (
            self._deepstack_buffers
            and self._deepstack_buf_capacity >= num_tokens
            and self._deepstack_buffers[0].device == device
            and self._deepstack_buffers[0].dtype == dtype
        ):
            return
        self._deepstack_buffers = [
            torch.zeros(cap, self.visual_dim, device=device, dtype=dtype)
            for _ in range(self.deepstack_num_level)
        ]
        self._deepstack_buf_capacity = cap
    ...
    def _set_deepstack_input_embeds(
        ...
        if not self.use_deepstack:
            return
        # Shape ``[num_levels, num_tokens, visual_dim]``.
        num_tokens = deepstack_input_embeds.size(1)
        self._ensure_deepstack_buffers(num_tokens, deepstack_input_embeds)
        for idx in range(self.deepstack_num_level):
            self._deepstack_buffers[idx][:num_tokens].copy_(
    ...
    def _get_deepstack_input_embeds(
        ...
        if not self.use_deepstack or not self._deepstack_buffers:
            return None
        return IntermediateTensors(
            {
                f"deepstack_input_embeds_{idx}": self._deepstack_buffers[idx][
                    :num_tokens
                ]
                for idx in range(self.deepstack_num_level)
            }
    ...
    def _clear_deepstack_input_embeds(self, num_tokens: int) -> None:
        ...
        if num_tokens > 0:
            for idx in range(self.deepstack_num_level):
                self._deepstack_buffers[idx][:num_tokens].zero_()
    ...
    def embed_input_ids(
        ...
        *,
        ...
        # ``_require_is_multimodal`` lives in interfaces but isn't re-exported;
        # call site mirrors the base SupportsMultiModal.embed_input_ids.
        if is_multimodal is None:
            raise ValueError(
                "is_multimodal must be provided when "
                "multimodal_embeddings is provided."
            )
